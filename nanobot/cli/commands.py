...
def _make_provider(config: Config, bus=None, cron_service=None):
    ...
    # Claude Code CLI: full agent loop via claude-agent-sdk
    if provider_name == "claude_code":
        from nanobot.providers.claude_code_provider import ClaudeCodeProvider

        cc = config.providers.claude_code
        return ClaudeCodeProvider(
            cli_path=cc.cli_path,  # empty string → provider converts to None (auto-discover)
            default_model=model,
            permission_mode=cc.permission_mode,
            allowed_tools=cc.allowed_tools,
            disallowed_tools=cc.disallowed_tools,
            system_prompt=cc.system_prompt or None,
            cwd=cc.cwd or str(config.workspace_path),
            max_turns=cc.max_turns,
            env=cc.env,
            bridge_message_tool=cc.bridge_message_tool,
            bridge_cron_tool=cc.bridge_cron_tool,
...
def gateway(
    ...
    # Create cron service before provider so ClaudeCodeProvider can receive it
    cron_store_path = get_cron_dir() / "jobs.json"
    cron = CronService(cron_store_path)

    provider = _make_provider(config, bus=bus, cron_service=cron)
